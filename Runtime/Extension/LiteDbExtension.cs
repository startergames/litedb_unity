...
namespace LiteDB {
    public static class LiteDbExtension {
        ...
        public static ILiteCollection<T> GetCollectionAutoInclude<T>(this LiteDatabase db, int maxDepth = 5) {
                    ...
                    .Select(member => {
                        return member switch {
                            PropertyInfo { PropertyType: { IsArray: true } } property => (
                                path: $"{prefix}.{property.Name}[*]", type: property.PropertyType.GetElementType()),
                            PropertyInfo property => (path: $"{prefix}.{property.Name}", type: property.PropertyType),
                            FieldInfo { FieldType: { IsArray: true } } field => (path: $"{prefix}.{field.Name}[*]",
                                type: field.FieldType.GetElementType()),
                            FieldInfo field => (path: $"{prefix}.{field.Name}", type: field.FieldType),
